...
import { formatDate, formatDay } from '~/utils/time';
import { getContract, prepareContractCall } from 'thirdweb';
import { client } from '~/app/client';
import { sepolia } from 'thirdweb/chains';
import { CONTRACT_ADDRESS } from '~/constants/address';
import { convertHash } from '~/utils/converHash';
import { convertBidStatus } from '~/utils/convertStatus';

export default function BidCards(p: { bids: BidRsp[] }) {
      ...
      {p.bids ? (
        <div className="pt-4 pl-3 py-4 bg-white rounded-xl dark:bg-zinc-900">
          <div className={'flex justify-between max-w-6xl'}>
            <span className="font-extrabold text-3xl text-gray-700 ml-3 pb-2 dark:text-gray-50">{`Track Your Bid`}</span>
          </div>
          <div className="grid grid-cols-4 gap-6">
            {p.bids.map(bid => (
              <a
                key={bid.biddingID}
                href={`/bid-tracker/${bid.biddingID}`}
                ...
                <div className="py-2">
                  <div className="flex items-center justify-center">
                    {bid.biddingStatus == '0' ? <span className="bg-green-200 rounded-xl px-2 py-1 font-bold">{convertBidStatus(bid.biddingStatus)}</span> : null}

                    {bid.biddingStatus == '1' ? <span className="bg-green-300 rounded-xl px-2 py-1 font-bold">{convertBidStatus(bid.biddingStatus)}</span> : null}

                    {bid.biddingStatus == '2' ? <span className="bg-blue-300 rounded-xl px-2 py-1 font-bold">{convertBidStatus(bid.biddingStatus)}</span> : null}

                    {bid.biddingStatus == '3' || bid.biddingStatus == '4' ? <span className="bg-yellow-300 rounded-xl px-2 py-1 font-bold">{convertBidStatus(bid.biddingStatus)}</span> : null}

                    {bid.biddingStatus == '5' ? <span className="bg-gray-300 rounded-xl px-2 py-1 font-bold ">{convertBidStatus(bid.biddingStatus)}</span> : null}
                  </div>

                  <p className="mt-3 items-center text-center bg-white text-gray-600 text-xs sm:text-sm rounded-xl">{formatDate(bid?.biddingTime)}</p>
                  <p
                    ...
                    onClick={e => {
                      e.preventDefault();
                      window.open(`https://sepolia.etherscan.io/tx/${bid?.hash}`, '_blank');
                    }}>
                    {convertHash(bid?.hash)}
                  </p>
                </div>
              </a>
            ))}
